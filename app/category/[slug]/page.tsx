...
const CATEGORY_SEO: Record<string, {
  ...
  business: {
    ...
    faqs: [
      ...
      { q: '我是新手管理者，怎么用AI快速上手？', desc: '先从会议纪要和任务拆解开始。① 每次会议录音转文字后让AI出「3段式纪要：结论+行动项+后续跟进」；② 每周工作：让AI把你的大目标拆解为具体的周任务+验收标准。坚持2个月，管理能力肉眼可见升级。' },
  ...
  career: {
    ...
    faqs: [
      { q: 'AI优化的简历会不会被HR看出来？会不会算作弊？', desc: '不算作弊。简历优化的行业标准服务一直存在（比如简历代写），AI只是把这个服务民主化了。关键红线：经历绝对真实（只优化表达不编造），面试问你简历上写的每一个点你都能详细讲出来。你要是做不到，说明AI加的东西你不该加。' },
      { q: '跨部门沟通遇到「甩锅侠」，AI能帮我写邮件吗？', desc: '当然。告诉AI前因后果+对方甩了什么锅+你有什么证据（聊天记录截图/邮件/文档），然后让它写一封「事实清楚+不带情绪+责任界定明确+有下一步行动建议」的邮件。关键原则：邮件永远只写事实和方案，不写情绪和指责。' },
      { q: '想跳槽但不敢裸辞，AI能帮我做什么？', desc: '4件事：① 每周花2小时让AI帮你做市场扫描（同岗位JD变化+技能趋势+薪资变化）；② 悄悄优化好简历+面试脚本；③ 让AI帮你做「跳槽准备Checklist」（作品集/项目复盘/推荐人/离职交接计划）；④ 拿到offer后让AI帮你做薪资谈判+离职沟通脚本。' },
    ],
    useCases: [
